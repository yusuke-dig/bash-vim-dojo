...
「Vim 基本操作リファレンス（毎日掲載）」の規約：
- 今日のテーマと関係なく、毎回同じ内容を掲載する
- 移動・編集・保存・終了・検索など、日常的に使う基本操作を網羅する
- テーブル形式で「コマンド ／ 動作」を一覧にする
- 内容は固定（以下のリストを毎回そのまま使う）：
...
- **厳禁**: `# YOUR CODE HERE` の後に解答コードを書かない。コメントと `# YOUR CODE HERE` だけで終わること。解答はユーザーが自分で書く。
